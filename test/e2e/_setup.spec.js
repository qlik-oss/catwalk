...
after(async () => {
  ...
  await nyc.report();
  if (!coverageFlag) {
    console.log('\u001b[31m The code hasn´t been instrumented! Please build with `npm run parcel:build` and start the server with `npm run server:start`\u001b[0m'); // eslint-disable-line no-console
  }
